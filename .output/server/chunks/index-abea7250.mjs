import { toRef, defineComponent, ref, computed, watch, nextTick, provide, reactive, onUpdated, openBlock, createElementBlock, normalizeClass, unref, createElementVNode, normalizeStyle, createBlock, resolveDynamicComponent, withCtx, renderSlot, createCommentVNode, shallowReactive, getCurrentInstance, onServerPrefetch, inject, getCurrentScope, onScopeDispose, warn, isVNode, Fragment, createVNode, Transition, withDirectives, vShow } from 'vue';
import { b as useNuxtApp, d as defineStore, c as createError } from './server.mjs';
import { isString as isString$1, isObject, hasOwn, isFunction } from '@vue/shared';
...
const useBlogStore = defineStore("blog", () => {
  const tag = ref({ _id: "-1", tagName: "\u5168\u90E8" });
  return {
    tag
  };
});
const GET_ARTICLES = "/articles/list";
const GET_ARTICLE_DETAIL = "/articles/detail";
const GET_TAGS = "/tags/list";
var ResponseErrorCode = /* @__PURE__ */ ((ResponseErrorCode2) => {
  ResponseErrorCode2[ResponseErrorCode2["Success"] = 0] = "Success";
  ResponseErrorCode2[ResponseErrorCode2["Error"] = 1] = "Error";
...
const service = $fetch.create({
  ...
  onRequest({ options }) {
    var _a;
    if (((_a = options.method) == null ? void 0 : _a.toLowerCase()) === "get")
      delete options.body;
  },
  ...
  onResponse(response) {
    const { code, msg, data } = response.response._data;
  ...
  onResponseError(error) {
    var _a;
    errorCreate(`${(_a = error.error) == null ? void 0 : _a.message}`);
  }
});
...
async function getCaptured(url, params = {}, config = {}) {
  return request(url, {
    ...config,
    method: "get",
    params
